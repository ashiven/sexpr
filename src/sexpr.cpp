...
#include "mim/plug/sexpr/autogen.h"
...
public:
    ...
    Emitter(World& world, std::ostream& ostream, bool typed = false, bool slotted = false)
        : Super(world, "sexpr_emitter", false, true)
        , ostream_(ostream) {
        typed_            = typed;
        slotted_          = slotted;
        types_enabled_    = true;
...
private:
    ...
    /// Recursively emits code for @p def into the BB it is scheduled to.
    std::string emit(const Def* def) {
        ...
        auto res = locals_[def] = emit_bb(bb, def);
        assert(!res.empty());
        return res;
    }
...
void Emitter::visit(const Nest& nest) {
    ...
    auto muts = Scheduler::schedule(nest);
    ...
    // A closed Lam is never a member of another Lam's Nest, so no BB outlives its function.
    lam2bb_.clear();
}
...
std::string Emitter::emit_bb(BB& bb, const Def* def) {
    ...
    if (def->isa_imm<Lam>()) {
        ...
        fe::throwf("SExpr backend: unhandled def `{}` of type `{}`", def, def->type());
        fe::unreachable();
    }
...
extern "C" MIM_EXPORT void mim_sexpr_emit(World& world, std::ostream& ostream, bool typed, bool slotted) {
    Emitter(world, ostream, typed, slotted).run();
}

/// Pipeline phase for `sexpr.emit`.
...
public:
    ...
    void start() override {
        auto name = world().name() ? world().name().str() : std::string("a");
        auto out  = Out(std::string(arg_value(args(), "o", "output").value_or(name + ".sexpr")));
        mim_sexpr_emit(world(), *out.os(), arg_flag(args(), "types"), arg_flag(args(), "slotted"));
    }
};
...
static void reg_phases(Flags2Phases& phases) { Phase::hook<plug::sexpr::emit, plug::sexpr::Emit>(phases); }

// clang-format off
static constexpr PluginArg known_args[] = {
    {"o=<file>, output=<file>", "Writes the symbolic expression to `<file>` instead of the default `<world>.sexpr`/`a.sexpr`; `<file>` may be `-` for stdout."},
...
// MIM_PLUGIN_SYM stringifies its argument, so this name must stay unqualified.
using namespace mim::plug::sexpr;
static const PluginSym known_syms[] = {
    MIM_PLUGIN_SYM(mim_sexpr_emit),
};

MIM_PLUGIN_ENTRY(sexpr) {
